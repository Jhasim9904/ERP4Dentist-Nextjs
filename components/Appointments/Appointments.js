...
useEffect(() => {
  ...
  const filteredPatients = patients.filter((p) =>
    `${p.name} ${p.phone} ${p.doctor}`.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="appointment-container">
      <div className="appointment-header d-flex align-items-center justify-content-between flex-wrap">
        <div className="d-flex align-items-center">
          ...
          <span className="count-badge">{patients.length.toString().padStart(2, "0")}</span>
        </div>
        ...
        <div className="d-flex align-items-center flex-wrap" style={{ gap: "25px" }}>
          ...
          <button className="btn btn-primary add-btn d-flex align-items-center" onClick={() => setShowModal(true)}>
